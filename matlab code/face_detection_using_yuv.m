...
face_original = imread('D:\study\MTech_2nd_sem\frame_images\733.jpg');
...
face_idx = find(face_area > (.26)*max(face_area)); %only shows indices ofregions that are faces
...
hold on
plot(centroids(:,1), centroids(:,2), 'b*', 'MarkerSize', 1)
hold off
